...
const orderController = {
    place_order: async (req, res) => {
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            if (!Array.isArray(productList) || productList.length === 0) {
                console.log('Product list is missing or empty for product:', products[i]);
                return res.status(400).json({ message: 'Product list is missing or empty' });
            }

            for (let j = 0; j < productList.length; j++) {
                const { productInfo, quantity } = productList[j];

                if (!productInfo || !quantity) {
                    console.log('Skipping invalid product:', productList[j]);
                    continue;
                }
                ...
                if (!sellerId || !sellerPrice) {
                    console.log('Missing sellerId or price for product:', products[i]);
                    return res.status(400).json({ message: 'Missing sellerId or price for one or more products' });
                }
                ...
                for (let k = 0; k < productList.length; k++) {
                    ...
                    if (!productInfo || !quantity) {
                        console.log('Skipping invalid product in storeProducts:', productList[k]);
                        continue;
                    }
        ...
        try {
            ...
            authorOrderData = authorOrderData.map(data => ({ ...data, orderId: order.id }));
            await authOrderModel.insertMany(authorOrderData);

            setTimeout(() => {
                console.log('Payment check for order:', order.id);
            }, 15000);
